...
🖼️ **Image needed:** a timeline showing ONE connection opened once, then multiple `data:`
events flowing server→client over time on that same connection, contrasted with long polling's
"open, close, reopen" cycle. Search: "server sent events vs websocket diagram".
